...
spinlock_t khyt1331_lock = SPIN_LOCK_UNLOCKED;       /* spin lock for busy check */
...
irqreturn_t khyt1331_interrupt(int irq, void *dev_id, struct pt_regs *regs);
...
static int khyt1331_open(struct inode *inode, struct file *file)
{
   ...
   spin_lock_irqsave(&khyt1331_lock,flags);

   if (khyt1331_in_use) {
      /* return if device already open */
      spin_unlock_irqrestore(&khyt1331_lock,flags);
...
#ifdef DEBUG
         printk(KERN_DEBUG "khyt1331: interrupt crate %d, station %d\n", i, enclam);
#endif
         /* disable interrupts */
         OUTP(adr + 10, 40);
...
int khyt1331_read(struct file *filep, char *buf, size_t count, loff_t * ppos)
{
   while (!interrupt_info) {
      ...
      wait_event_interruptible(waitq,interrupt_info);
   }
...
int init_module(void)
{
   ...
   if (register_chrdev(MOD_MAJOR, "hyt1331", &khyt1331_fops)) {
   ...
   if (dev != NULL) {
      ...
      if ( ! request_region(io_base, 0x40, "khyt1331") ) {
...
void cleanup_module(void)
{
   ...
   if (unregister_chrdev(MOD_MAJOR, "hyt1331"))
